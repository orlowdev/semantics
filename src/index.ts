...
export const getChangelog = (changes: CommitInterface[]): string => {
  ...
  return CommitTypes.map((ct: ICommitType) => {
    ...
    if (!ct.display && commitsOfThatType.length) {
      Messenger.info(`Skipping ${Iro.blue(Iro.bold(`${commitsOfThatType.length} ${ct.type}`))} commits`);
      return substring;
    }

    if (commitsOfThatType.length) {
      Messenger.success(`Adding ${Iro.green(Iro.bold(`${commitsOfThatType.length} ${ct.type}`))} commits to the changelog`);
    }
